(function ($) {
    ...
    $('.slider-range-price').each(function () {
        var min = parseInt($(this).data('min')); // Ubah ke tipe data integer
        var max = parseInt($(this).data('max')); // Ubah ke tipe data integer
        var unit = $(this).data('unit');
        var value_min = parseInt($(this).data('value-min')); // Ubah ke tipe data integer
        var value_max = parseInt($(this).data('value-max')); // Ubah ke tipe data integer
        ...
        $(this).slider({
            ...
            min: min,
            max: max,
            values: [value_min, value_max], // Gunakan value_min dan value_max yang diambil dari data atribut
            slide: function (event, ui) {
                var result = label_result + " " + unit + ui.values[0] + ' - ' + unit + ui.values[1];
                t.closest('.slider-range').find('.range-price').html(result);
                filterProductsByPrice(ui.values[0], ui.values[1]); // Perbarui dengan ui.values[0] dan ui.values[1]
            }
        });
    });
